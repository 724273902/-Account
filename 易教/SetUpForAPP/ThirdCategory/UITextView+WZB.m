...
//    Method desaloc =class_getInstanceMethod(self.class, NSSelectorFromString(@"dealloc"));
//    Method myDelloc =class_getInstanceMethod(self.class, @selector(myDealloc));
//    method_exchangeImplementations(dealoc, desaloc);
}
...
{
       CGFloat mas =wzb_maxHeight;
    
    // 如果传入的最大高度小于textView本身的高度，则让最大高度等于本身高度

    if(wzb_maxHeight <self.frame.size.height){
        mas =self.frame.size.height;
    }
    objc_setAssociatedObject(self, WZBTextViewMaxHeightKey, [NSString stringWithFormat:@"%lf",mas], OBJC_ASSOCIATION_COPY_NONATOMIC);
