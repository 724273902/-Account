...
- (void)setTopicViewModel:(ZXViewModel *)topicViewModel
{
    ...
    // 1, playcount
    self.voicePlayCountLabel.text = [NSString stringWithFormat:@"%zd播放", topicViewModel.topicItem.playfcount];
    ...
    if(_voiceLengthLabel == nil)
    {
      ...
      voiceLengthLabel.font = [UIFont systemFontOfSize:12];
        ...
        [voiceLengthLabel mas_makeConstraints:^(MASConstraintMaker *make) {
            ...
            make.left.offset(YPScreenW*5/6);
        }];
...
- (UIImageView *)pictureImageView
{
    ...
    {
        UIImageView *pictureImageView = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, YPScreenW, YPScreenH)];
     ...
     //   pictureImageView.frame=self.frame;
...
- (UIButton *)voicePlayButton
{
    if(_voicePlayButton == nil)
    {
      ...
      //  btn.frame= CGRectMake(self.centerX, self.centerY, 60, 60);
...
- (void)playVideo:(UIButton *)btn
{
    ...
    static NSInteger NJPlayer = 2;

    switch (NJPlayer) {
        case 1:
        {
            AVPlayerItemController *videoVc = [[AVPlayerItemController alloc] init];
            videoVc.videoUrl = self.topicViewModel.topicItem.videoUrl.absoluteString;
...
//            [self.viewController.navigationController pushViewController:playerVc animated:YES];
        ...
        case 2:
        {
            AVVideoViewController *video =[AVVideoViewController new];
            video.videoUrl=self.topicViewModel.topicItem.videoUrl.absoluteString;

             [self.viewController presentViewController:video animated:NO completion:nil];
