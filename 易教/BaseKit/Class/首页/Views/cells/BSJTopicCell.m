...
//    [self.headerImageView sd_setImageWithURL:topicViewModel.topic.profile_image placeholderImage:[UIImage imageNamed:@"defaultUserIcon"]];
    [self.headerImageView sd_setImageWithURL:topicViewModel.topicItem.profile_image placeholderImage:nil];
    
    
    self.screenNameLabel.text = topicViewModel.topicItem.name;
    
    self.creatTimeLabel.text = topicViewModel.creatTime;
    
    self.contentTextLabel.text = topicViewModel.topicItem.text;
    
    self.isSinaVipImageView.hidden = !topicViewModel.topicItem.isSina_v;
    ...
    if (topicViewModel.topicItem.type == BsjTopicPicture) {
    ...
    }else if (topicViewModel.topicItem.type == BsjTopicTypeVoice)
    {
    ...
    }else if (topicViewModel.topicItem.type== BsjTopicWords)
    {
        _pictureView.hidden = YES;
        _voiceView.hidden = YES;
        _videoView.hidden = YES;
        
    }else if (topicViewModel.topicItem.type == BsjTopicTypeVideo)
    {
    ...
    // 热门评论
    if (topicViewModel.topicItem.topCmts.count) {
        ...
        self.topicViewModel.cmtComment = ^(ZXuser *user, topicComment *comment) {
            NSLog(@"%@",user.username);
